...
This styleguide is heavily influenced by AirBnB's guide. For a tl;dr of our changes, check out the [tl;dr folder](/tldrs).
...
## Comparison Operators & Equality
  ...
  - [15.3](#comparison--shortcuts) Always use explicit statements

  > Why? Due to JavaScript's typing (or lack thereof), if statements are implicitly converted to Boolean which can cause unexpected behavior if code is non-specific.
  ...
  - [15.6](#comparison--nested-ternaries) Ternaries should not be nested and generally be single line expressions. eslint: [`no-nested-ternary`](https://eslint.org/docs/rules/no-nested-ternary.html)
    ...
    // split into a nicer looking statement. Truthy && statements evaluate to their last member, || statements evaluate to the first truthy member
    const maybeNull = (maybe1 > maybe2 && "bar") || (value > value2 && "baz") || null
    ```
  ...
  - [15.7](#comparison--unneeded-ternary) Avoid unneeded ternary statements. eslint: [`no-unneeded-ternary`](https://eslint.org/docs/rules/no-unneeded-ternary.html)
    ...
    // good
    const foo = a || b;
    // coerces non-Boolean to Boolean
    const bar = !!c;
    const baz = !c;
    ```
...
## Comments
  ...
  - [18.1](#comments--multiline) Use `/** ... */` for multi-line comments with an empty line after. Multi-line comments should never be inside a function or class, they must always be outside.
  ...
  - [18.4](#comments--actionitems) Do not use TODO or FIXME. Either do it now or put it in Jira.
...
## Semicolons
  ...
  <a name="semicolons--rnotequired"></a><a name="20.1"></a>
  - [21.1](#semicolons--notrequired) **No.** eslint: [`semi`](https://eslint.org/docs/rules/semi.html)
...
## Naming Conventions
  ...
  - [23.10](#naming--uppercase) You may optionally uppercase a constant only if it (1) is exported, (2) is a `const` (it can not be reassigned), and (3) the programmer can trust it (and its nested properties) to never change.
    ...
    **Debate whether to go all uppercase or adopt Werle's method of prepending with `k`
